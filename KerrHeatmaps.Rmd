...
```{r filter and int and heatmap function}
rawData <- kerr
stationFilter <- '4AROA018.36'
yearFilter <- '2018'
...
int_heatmap <- function(rawData, 
  ...
  if(!(as.character(stationCol1)[2] %in% names(rawData)) | 
  ...
  contour.heatmap(int_year, reverseColorPalette = reverseColorPalette, 
                  criteria = criteria)
...
int_heatmap(kerr, stationFilter = '4AROA018.36', yearFilter = 2016, 
            stationCol = FDT_STA_ID,
            dateCol = Date, 
            depthCol = Depth, 
            parameterCol = `Do Optical`, #`Temp Celsius`, 
            criteriaCol = `Dissolved Oxygen Min (mg/L)`, #`Max Temperature (C)`,
            dateFormat= '%Y-%m-%d',
...
int_heatmap <- function(rawData, 
                        ...
                        dateFormat){
  # First check to make sure station and station column exist in dataset
  if(!(stationCol %in% names(rawData)) | 
     
     rawData[,deparse(stationCol)]
     x[, deparse(col_name)]
     
     stationFilter %in% rawData$quote(stationCol)
     unique(select(rawData, stationCol))
  
  # filter dataset by desired station
  dat <- filter(rawData, get(stationCol) == stationFilter)
  return(dat)


z <- int_heatmap(kerr, stationFilter = '4AROA018.36',  stationCol = 'FDT_STA_ID')
