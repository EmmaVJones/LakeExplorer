...
mendota.df <- read_csv("data/learning/mendota_temp.csv")
...
ggplotly(ggplot(mendota_2013.df, aes(x = date, y = depth, z = wtemp, fill = wtemp)) +
  geom_raster() +
...
#plot_ly(data= mendota_2013.df, x=~date, y = ~depth, color = ~wtemp, fill = ~wtemp)
